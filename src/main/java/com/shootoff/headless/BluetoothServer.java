...
class BluetoothServer implements HeadlessServer {
    ...
    BluetoothServer(QRCodeListener qrListener) {
        ...
        if (!addressQRCode.isPresent())
            return;

        if (qrListener != null)
            qrListener.qrCodeCreated(addressQRCode.get());
    }
    ...
    private Optional<String> getLocalAddress() {
        try {
            ...
            for (int i = 0; i < originalAddress.length(); i++) {
                ...
                if (i > 0 && i < originalAddress.length() - 1 && i % 2 != 0)
                ...
                while (open.get()) {
                    try {
                        ...
                        if (messageListener != null)
                            messageListener.messageReceived(Message.fromJson(lineRead));
                    } catch (IOException e) {
