...
public class ProjectorTrainingExerciseBase extends TrainingExerciseBase {
     ...
     * events and other javafx functions that need true coordinates rather than
     * scaled ones.
     * 
     * @param point
    ...
    public boolean isPerspectiveInitialized() {
        return arenaPane.getPerspectiveManager().isPresent() && arenaPane.getPerspectiveManager().get().isInitialized();
    }
     ...
     */
    public boolean setTargetDistance(Target target, int currentRealWidth, int currentRealHeight, int desiredDistance) {
        if (!isPerspectiveInitialized())
            return false;
