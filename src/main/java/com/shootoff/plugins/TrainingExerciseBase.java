...
public abstract class TrainingExerciseBase {
    ...
    public void getDelayedStartInterval(final DelayedStartListener listener) {
        if (listener == null)
            throw new IllegalArgumentException("Delayed start listener must be non-null");

        if (haveDelayControls)
            return;
    ...
    public void getParInterval(final ParListener listener) {
        if (listener == null)
            throw new IllegalArgumentException("Par listener must be non-null");
    ...
    public void reset() {
        ...
        if (config.getExercise().isPresent())
            config.getExercise().get().reset(exerciseView.getTargets());
    }
