...
public class PluginEngine implements Runnable {
    ...
    private boolean registerPlugin(final Path jarPath) {
        ...
        if (existingPlugin.isPresent()) {
            ...
            if (VersionChecker.compareVersions(existingVersion, loadedVersion) == -1) {
                // Existing is older
                logger.debug(
                        "Registering plugin ({}, {}, {}, {}) is a newer duplicate of an "
                                + "already registered plugin ({}, {}, {}, {})",
                        registeringMetadata.getName(), registeringMetadata.getVersion(),
                        registeringMetadata.getCreator(), registeringPlugin.getJarPath(), existingMetadata.getName(),
                        existingMetadata.getVersion(), existingMetadata.getCreator(), existing.getJarPath());
                unregisterPlugin(existing);
            } else {
                // Existing is newer or the same, do nothing
                logger.debug(
                        "Registering plugin ({}, {}, {}, {}) is an older or same version duplicate of an "
                                + "already registered plugin ({}, {}, {}, {})",
                        registeringMetadata.getName(), registeringMetadata.getVersion(),
                        registeringMetadata.getCreator(), registeringPlugin.getJarPath(), existingMetadata.getName(),
                        existingMetadata.getVersion(), existingMetadata.getCreator(), existing.getJarPath());
    ...
    public Optional<Plugin> getPlugin(TrainingExercise trainingExercise) {
        for (final Plugin p : plugins) {
            if (p.getExercise().getInfo().equals(trainingExercise.getInfo()))
                return Optional.of(p);
        }
        ...
        while (watching.get()) {
            ...
            for (final WatchEvent<?> event : key.pollEvents()) {
                ...
                if (StandardWatchEventKinds.ENTRY_CREATE.equals(event.kind())) {
                    if (!registerPlugin(fqUpdatedFile))
                        continue;
