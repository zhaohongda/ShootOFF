...
public class ProjectorArenaPane extends AnchorPane implements CalibrationListener, Closeable {
    ...
    public void startCalibration() {
        ...
        if (mirroredArenaPane != null)
            mirroredArenaPane.mirroredStartCalibration();
    }
    ...
    private void cursorWarningToggle(boolean mouseEntered) {
        if (feedCanvasManager == null || calibrationManager == null || calibrationManager.isCalibrating())
            return;

        // If everything is still the same, return
        if (mouseEntered && showingCursorWarning && !calibrationManager.isCalibrating())
            return;
    ...
    public void calibrated(Optional<PerspectiveManager> perspectiveManager) {
        ...
        if (mirroredArenaPane != null)
            mirroredArenaPane.mirrorCalibrated(perspectiveManager);
    }
    ...
    public void targetAdded(Target target) {
        ...
        if (mirroredArenaPane != null)
            mirroredArenaPane.mirrorTargetAdded(target);
    }
    ...
    private void showPerspectiveUsageMessage() {
        if (config.showedPerspectiveMessage() || config.inDebugMode())
            return;
        ...
        if (shootOffStage != null)
            recalibrationAlert.initOwner(shootOffStage);
