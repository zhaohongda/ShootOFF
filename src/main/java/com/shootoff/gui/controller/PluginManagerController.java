...
public class PluginManagerController {
    ...
    @FXML
    private VBox pluginManagerPane;
    @FXML
    private TableView<PluginMetadata> pluginsTableView;
    ...
    public void init(PluginEngine pluginEngine, Stage pluginManagerStage) {
        ...
        final TableColumn<PluginMetadata, String> descriptionCol = new TableColumn<>("Description");
            ...
            if (!empty) {
                ...
                actionButton.setOnAction((e) -> {
                    if (downloadTask.isPresent()) {
                        ...
                        final Optional<Plugin> installedPlugin = metadata
                                .findInstalledPlugin(pluginEngine.getPlugins());
    ...
    private Optional<Task<Boolean>> installPlugin(PluginMetadata metadata, Runnable successAction,
        ...
        final File downloadedFile = new File(
                String.format("%s%s%s-%s-%s.jar", System.getProperty("shootoff.home"), File.separator,
                        metadata.getName().replaceAll("\\s", "_"), metadata.getVersion(), metadata.getCreator()));
    ...
    private boolean uninstallPlugin(final Plugin plugin) {
        if (!plugin.getJarPath().toFile().delete()) {
            logger.error("Failed to uninstall file {} -- if Windows it's because the OS has a write lock on the file",
                    plugin.getJarPath().toString());
            return false;
        }
    ...
    protected Set<PluginMetadata> parsePluginMetadata(final String pluginMetadata) {
        ...
        try (InputStream xmlInput = new ByteArrayInputStream(pluginMetadata.getBytes("UTF-8"))) {
            ...
            if (logger.isWarnEnabled())
                logger.warn("Failed to download complete plugin metadata, no or "
