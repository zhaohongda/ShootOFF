...
public class TargetView implements Target {
    ...
    public void toggleSelected() {
        ...
        for (final Node node : getTargetGroup().getChildren()) {
            if (!(node instanceof TargetRegion))
                continue;
        ...
        if (selectionListener != null)
            selectionListener.targetSelected(this, isSelected);
    }
                        ...
                        if (adjustedX < 0 || adjustedY < 0) {
                            logger.debug(
                                    "An adjusted pixel is negative: Adjusted ({}, {}), Original ({}, {}), "
                                            + " nodeBounds.getMin ({}, {})",
                                    adjustedX, adjustedY, x, y, nodeBounds.getMaxX(), nodeBounds.getMinY());
                            return Optional.empty();
                        }
