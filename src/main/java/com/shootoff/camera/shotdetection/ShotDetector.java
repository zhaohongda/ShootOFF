    ...
    public boolean addShot(ShotColor color, double x, double y, long timestamp, boolean scaleShot) {
        ...
        if (config.isAdjustingPOI()) {
            if (logger.isTraceEnabled()) {
                logger.trace("POI Adjustment: x {} y {}", config.getPOIAdjustmentX().get(),
                        config.getPOIAdjustmentY().get());
                logger.trace("Adjusting offset via POI setting, coords were {} {} now {} {}", x, y,
                        x + config.getPOIAdjustmentX().get(), y + config.getPOIAdjustmentY().get());
            }
